...
# Configuração de Log (Único e Acumulativo)
logging.basicConfig(
    filename=PASTA_LOGS / "historico_geral_execucao.log",
    filemode='a',
    level=logging.INFO,
...
def encontrar_cabecalho_correto(df, colunas_esperadas):
    if not colunas_esperadas: return df
...
def tratar_dataframe(df, config, metadados):
    ...
    colunas_valor = [col for col in df.columns if 'Valor' in str(col) or 'Total' in str(col) or 'Realizado' in str(col)]
    for col in colunas_valor: df[col] = df[col].apply(limpar_valor_monetario)
    ...
    col_val = next((c for c in df.columns if 'Valor' in str(c) or 'Total' in str(c)), df.columns[1] if len(df.columns) > 1 else df.columns[0])
...
# --- FUNÇÃO DE SALVAMENTO INTELIGENTE (APPEND) ---
...
def salvar_com_append_preservando_formatacao(df_novos, caminho_arquivo, nome_aba):
    ...
    try:
        ...
        try:
            ...
            col_val_ex = next((c for c in df_existente.columns if 'Valor' in str(c) or 'Total' in str(c)), df_existente.columns[2])
...
def aplicar_estilo_inicial(ws, sheet_name):
    ...
    estilo = TableStyleInfo(name="TableStyleLight1", showFirstColumn=False,
...
def executar_pipeline():
    ...
    try:
        ...
        for fornecedor, config in CONFIG_FORNECEDORES.items():
            ...
            for msg in mensagens:
                try:
                    ...
                    try:
                        if msg.ReceivedTime.replace(tzinfo=None) < DATA_INICIO_LEITURA:
                            continue
                    except: continue
                    ...
                    # Fallback Positivo
                    if tabela_alvo is None and fornecedor == "Positivo" and len(todas_tabelas) > 0:
                        tabela_alvo = max(todas_tabelas, key=len)
        ...
        # --- ATUALIZAÇÃO NO LOOP DE MOVER (CRIA PASTA DO FORNECEDOR) ---
